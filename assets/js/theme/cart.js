...
export default class Cart extends PageManager {
  ...
  cartUpdate($target) {
    ...
    api.cart.itemUpdate(itemId, newQty, (_error, response) => {
      this.$overlay.hide()
  ...
  cartRemoveItem(itemId) {
    this.$overlay.show()
    api.cart.itemRemove(itemId, (_error, response) => {
      if (response.data.status === 'succeed') {
  ...
  cartEditOptions(itemId, productId) {
    ...
    api.productAttributes.configureInCart(
      itemId,
      options,
      (_error, response) => {
  ...
  refreshContent(remove) {
    ...
    api.cart.getContent(options, (_error, response) => {
  ...
  bindPromoCodeEvents() {
    ...
    $couponForm.on('submit', (event) => {
      ...
      api.cart.applyCode(code, (_error, response) => {
  ...
  bindGiftCertificateEvents() {
    ...
    $certForm.on('submit', (event) => {
      ...
      api.cart.applyGiftCertificate(code, (_error, resp) => {
  ...
  bindGiftWrappingEvents() {
    ...
    $('[data-item-giftwrap]').on('click', (event) => {
      ...
      api.cart.getItemGiftWrappingOptions(
        itemId,
        options,
        (_error, response) => {
          modal.updateContent(response.content)
