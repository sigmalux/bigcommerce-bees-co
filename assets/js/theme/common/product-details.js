...
export default class ProductDetails extends ProductDetailsBase {
  constructor($scope, context, productAttributesData = {}) {
    ...
    if ((isEmpty(productAttributesData) || hasDefaultOptions) && hasOptions) {
      ...
      api.productAttributes.optionChange(
        $productId,
        $form.serialize(),
   ...
   * @param {HTMLElement} $swatch Swatch HTML element
   * @param {HTMLElement} $swatchGroup Swatch group HTML element
   */
  ...
  showProductImage(image) {
    if (isPlainObject(image)) {
      const zoomImageUrl = tools.imageSrcset.getSrcset(
        image.data,
      ...
      const mainImageUrl = tools.imageSrcset.getSrcset(
        image.data,
      ...
      const mainImageSrcset = tools.imageSrcset.getSrcset(image.data)
  ...
  addProductToCart(event, form) {
    ...
    // Add item to cart
    api.cart.itemAdd(normalizeFormData(new FormData(form)), (err, response) => {
   ...
   * @param {Function} onComplete onComplete function
   */
